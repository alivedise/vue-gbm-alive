  ...
  <div>
   ...
   >
      <template v-slot:item.image="{ item }">
        <div class="p-2">
          <v-img :src="item.icon" :alt="item.machineName" height="30px" width="30px"></v-img>
        </div>
      </template>
  ...
  computed: {
    mappedParts() {
      return this.parts.map((part) => ({
        ...
        accumluatedRangeAttack: Math.floor((+part.range) + (+part.rangeDefense * 0.4)),
      }));
    },
  },
